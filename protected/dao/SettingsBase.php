...
abstract class SettingsBase extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('is_free_all', 'numerical', 'integerOnly'=>true),
			array('app_name', 'length', 'max'=>255),
			array('fetch_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, fetch_time, app_name, is_free_all', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('is_free_all',$this->is_free_all);
